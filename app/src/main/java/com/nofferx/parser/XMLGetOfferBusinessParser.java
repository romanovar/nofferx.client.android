 ...
 */
public class XMLGetOfferBusinessParser implements XMLParser {
    ...
    public XMLGetOfferBusinessParser(int offerId, ISubject h)
    {
        hp = new HTTPConnector();
        String httpRequest = "business/offer/getAll/offer=" + offerId;
        this.h = h;
        ...
        try{
            ...
            for(int i =0 ; i < nodes.getLength(); i++) {
                ...
                for (int z = 0; z < n.getLength(); z++) {
                        // Loops through history and offer
                        String k = n.item(z).getNodeName();
                        String v = n.item(z).getTextContent();
                        ah.put(k, v);
                    }
                historyList.add(ah);
            }
